<template>
    <div>
        ...
        <section class="pretest-results-sec content">
            ...
            <div class="card" v-for="(data,index) in post.data" :key="data.attempt_answer_id">
                <header class="card-header" variant="primary">
                   <strong> Q {{index}} : {{data.question}} </strong>
                </header>
                
                    <b-list-group>
                        <b-list-group-item v-for="(ans,idx) in data.answers" :key="idx" :variant="getVariant(data, idx)">
        ...
        methods: {
            getVariant: function(data, idx) {
             ...
             fetchData: function() {
               ...
               axios.get('/pre-result/view/'+this.$route.params.id + '?type='+this.type)
                .then(response => {
                    loader.hide();
